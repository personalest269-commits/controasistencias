...
class PgAsistenciaReportesEmpresaController extends Controller
{
    ...
    private function empresasVigentes()
    {
        return PgEmpresa::query()
            ->where(function ($q) {
                $q->whereNull('estado')->orWhere('estado', '<>', 'X');
            })
            ->orderBy('nombre')
    ...
    public function Index(Request $request)
    {
        ...
        $empresas = $this->empresasVigentes();
        ...
        if ($empresaId) {
            // Personas cuyo departamento pertenece a la empresa
            if (empty($deptIds)) {
        ...
        $resumenEmp = $this->buildResumenEmpresa($summary, $departamentos, $empresas);

        return view('PgAsistenciasEmpresa.reportes', [
            'desde' => $desde,
            'hasta' => $hasta,
            'empresaId' => $empresaId,
            'empresas' => $empresas,
            'summary' => $summary,
            'resumenEmp' => $resumenEmp,
        ]);
    }
    ...
    public function DetallePersona(Request $request, string $personaId)
    {
        ...
        return view('PgAsistenciasEmpresa.reporte_persona', [
            'persona' => $persona,
            'desde' => $desde,
            'hasta' => $hasta,
            'empresaId' => $empresaId,
    ...
    {
        [$desde, $hasta, $empresaId, $empresas, $departamentos, $personas] = $this->resolveFilters($request);
        $summary = $this->buildSummary($personas, $desde, $hasta);
        $resumenEmp = $this->buildResumenEmpresa($summary, $departamentos, $empresas);
        ...
        $fileName = 'reporte_asistencia_resumen_' . $desde . '_a_' . $hasta . ($empresaId ? ('_emp_' . $empresaId) : '') . '.xls';

        return response()
            ->view('PgAsistenciasEmpresa.export_xls_resumen', compact('desde', 'hasta', 'empresaId', 'empresas', 'summary', 'resumenEmp', 'logoUrl', 'nombreSistema'))
    ...
    {
        [$desde, $hasta, $empresaId, $empresas, $departamentos, $personas] = $this->resolveFilters($request);
        $detalle = $this->buildDetalleAll($personas, $desde, $hasta, $departamentos, $empresas);

        $logoUrl = PgConfiguracion::reportLogoUrl();
        $nombreSistema = (string) PgConfiguracion::valor('NOMBRE_SISTEMA', config('app.name', 'Control de Asistencia Municipal'));
        $fileName = 'reporte_asistencia_detalle_' . $desde . '_a_' . $hasta . ($empresaId ? ('_emp_' . $empresaId) : '') . '.xls';

        return response()
            ->view('PgAsistenciasEmpresa.export_xls_detalle', compact('desde', 'hasta', 'empresaId', 'empresas', 'detalle', 'logoUrl', 'nombreSistema'))
            ->header('Content-Type', 'application/vnd.ms-excel; charset=UTF-8')
            ->header('Content-Disposition', 'attachment; filename="' . $fileName . '"');
    }
    ...
    public function ExportPdfResumen(Request $request)
    {
        ...
        return view('PgAsistenciasEmpresa.export_pdf', [
            'payload' => $payload,
    ...
    {
        [$desde, $hasta, $empresaId, $personaId, $empresas, $departamentos, $personasSelect, $personas] = $this->resolveFiltersDiaEvento($request);

        $data = $this->buildDiaEvento($personas, $desde, $hasta);

        return view('PgAsistenciasEmpresa.reporte_dia_evento', [
            'desde' => $desde,
            'hasta' => $hasta,
            'empresaId' => $empresaId,
            'empresas' => $empresas,
    ...
    public function ExportXlsDiaEvento(Request $request)
    {
        ...
        return response()->view('PgAsistenciasEmpresa.export_xls_dia_evento', [
            'desde' => $desde,
            'hasta' => $hasta,
            'empresaId' => $empresaId,
            'empresas' => $empresas,
    ...
    public function ExportPdfDiaEvento(Request $request)
    {
        ...
        return view('PgAsistenciasEmpresa.export_pdf_dia_evento', [
            'desde' => $desde,
            'hasta' => $hasta,
            'empresaId' => $empresaId,
            'empresas' => $empresas,
    ...
    {
        [$anio, $mes, $todosMeses, $empresaId, $personaId, $empresas, $departamentos, $personasSelect, $personas] = $this->resolveFiltersMes($request);

        $data = $this->buildMesCalendario($personas, $anio, $mes, $todosMeses);

        return view('PgAsistenciasEmpresa.reporte_mes', [
            'anio' => $anio,
            'mes' => $mes,
    ...
    public function ExportXlsMes(Request $request)
    {
        ...
        return response()->view('PgAsistenciasEmpresa.export_xls_mes', [
            'anio' => $anio,
            'mes' => $mes,
            'todosMeses' => $todosMeses,
            'empresaId' => $empresaId,
    ...
    public function ExportPdfMes(Request $request)
    {
        ...
        return view('PgAsistenciasEmpresa.export_pdf_mes', [
            'anio' => $anio,
            'mes' => $mes,
            'todosMeses' => $todosMeses,
            'empresaId' => $empresaId,
    ...
    private function resolveFiltersDiaEvento(Request $request): array
    {
        ...
        if ($empresaId) {
            if (empty($deptIds)) $personasSelectQ->whereRaw('1=0');
            else $personasSelectQ->whereIn('departamento_id', $deptIds);
        }
        ...
        if ($empresaId) {
            if (empty($deptIds)) $personasQ->whereRaw('1=0');
            else $personasQ->whereIn('departamento_id', $deptIds);
        }
        if ($personaId) {
        ...
        return [$desde, $hasta, $empresaId, $personaId, $empresas, $departamentos, $personasSelect, $personas];
    }
    ...
    private function resolveFiltersMes(Request $request): array
    {
        ...
        return [$anio, $mes, $todosMeses, $empresaId, $personaId, $empresas, $departamentos, $personasSelect, $personas];
    }
    ...
    private function buildResumenEmpresa(array $summaryPersona, $departamentos, $empresas): array
    {
        $depToEmp = [];
        foreach ($departamentos as $d) {
            $depToEmp[(string) $d->id] = (string) ($d->empresa_id ?? '');
        }

        $empName = [];
        foreach ($empresas as $e) {
            $empName[(string) $e->id] = (string) $e->nombre;
        }

        $groups = [];
        foreach ($summaryPersona as $r) {
            $depId = (string) ($r['departamento_id'] ?? '');
            $empId = $depId !== '' ? (string) ($depToEmp[$depId] ?? '') : '';
            $key = $empId !== '' ? $empId : '__SIN__';

            if (!isset($groups[$key])) {
                $groups[$key] = [
                    'empresa_id' => $empId !== '' ? $empId : null,
                    'empresa' => $empId !== '' ? ($empName[$empId] ?? $empId) : 'Sin empresa',
                    'totales' => ['convocados' => 0, 'asistidos' => 0, 'justificados' => 0, 'no_asistio' => 0],
                    'personas' => [],
                ];
            }
        ...
        uasort($groups, function ($a, $b) {
            return strcmp($a['empresa'], $b['empresa']);
        });

        return array_values($groups);
    }

    private function buildDetalleAll($personas, string $desde, string $hasta, $departamentos, $empresas): array
    {
        $depName = [];
        $depToEmp = [];
        foreach ($departamentos as $d) {
        ...
        foreach ($personas as $p) {
            ...
            $empId = $depId !== '' ? (string) ($depToEmp[$depId] ?? '') : '';
            $empDesc = $empId !== '' ? ($empName[$empId] ?? $empId) : 'Sin empresa';
    ...
    private function resolveFilters(Request $request): array
    {
        ...
        return [$desde, $hasta, $empresaId, $empresas, $departamentos, $personas];
    }
