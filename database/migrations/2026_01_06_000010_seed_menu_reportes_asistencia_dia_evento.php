...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!$reportes) {
            return; // si no existe, ya hay otras migraciones que lo crean
        }
        $reportesId = (string) $reportes->id;

        // Child: Asistencia por Día y Evento
        $child = DB::table('pg_opcion_menu')
            ->where('id_padre', $reportesId)
            ->where($titleCol, 'Asistencia por Día y Evento')
            ->first();

        if (!$child) {
            $data = [
                'id_padre' => $reportesId,
                $titleCol => 'Asistencia por Día y Evento',
                // La vista se resuelve por nombre de ruta en el template
                'url' => 'PgAsistenciasReporteDiaEvento',
                'orden' => 20,
            ];
