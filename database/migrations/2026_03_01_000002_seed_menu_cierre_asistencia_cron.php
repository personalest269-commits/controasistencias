...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // 1) Intentar colgar del menú "Gestión asistencia" si existe
        $padre = DB::table('pg_opcion_menu')
            ->whereNull('estado')
            ->where($titleCol, 'Gestión asistencia')
            ->first();

        // 2) Si no existe, colgar de "Gestión" (raíz)
        if (!$padre) {
            $padre = DB::table('pg_opcion_menu')->whereNull('id_padre')->where($titleCol, 'Gestión')->first();
        }
        if (!$padre) {
            return;
        }

        $menuId = $this->ensureMenu((string) $padre->id, 'Cierre asistencia (Cron)', 'PgCierreAsistenciaCronIndex', 99);
