...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!$reportes) {
            ...
            if (Schema::hasColumn('pg_opcion_menu', 'id')) {
                try {
                    ...
                    $reportesId = (string) $data['id'];
                } catch (Throwable $e) {
                    $reportesId = (string) DB::table('pg_opcion_menu')->insertGetId($data);
                }
            } else {
                $reportesId = (string) DB::table('pg_opcion_menu')->insertGetId($data);
            }
        ...
        // Child: reporte asistencia por departamento
        $child = DB::table('pg_opcion_menu')
            ->where('id_padre', $reportesId)
            ->where($titleCol, 'Asistencia por departamento')
            ->first();

        if (!$child) {
            $data = [
                'id_padre' => $reportesId,
                $titleCol => 'Asistencia por departamento',
                'url' => 'PgAsistenciasReportes',
                'orden' => 10,
            ];
            if (Schema::hasColumn('pg_opcion_menu', 'tipo')) {
                $data['tipo'] = 'M';
