...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $titleCol = Schema::hasColumn('pg_opcion_menu', 'titulo')
            ? 'titulo'
        ...
        $titulo = 'Asistencia por Día y Evento (Empresa)';
        $url = 'PgAsistenciasEmpresaReporteDiaEvento';
        ...
        foreach ($roles as $r) {
            ...
            $exists = DB::table('pg_opcion_menu_rol')
                ->where('id_opcion_menu', $childId)
                ->where($roleCol, $roleVal)
                ->exists();
            if ($exists) {
                continue;
            }
