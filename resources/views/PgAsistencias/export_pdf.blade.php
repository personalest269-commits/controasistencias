    ...
    <script>
        (async function () {
            ...
            function header(pageData) {
                ...
                doc.setFontSize(10);
                const subt = isDetalle ? 'Reporte detallado de asistencia (por día)' : 'Reporte general de asistencia por departamento';
            ...
            try {
                // Tablas
                if (!isDetalle) {
                    const grupos = payload.resumenDept || [];
                    let startY = headerH + 18;

                    grupos.forEach((g, idx) => {
                        const title = `${safeText(g.departamento)}  (Convocados: ${g.totales?.convocados ?? 0}, Asistidos: ${g.totales?.asistidos ?? 0}, Justificados: ${g.totales?.justificados ?? 0}, No asistió: ${g.totales?.no_asistio ?? 0})`;
                        doc.setFont('helvetica', 'bold');
                        doc.setFontSize(11);
                    ...
                    doc.autoTable({
                        startY: headerH + 18,
                        margin: { left: margin, right: margin, top: headerH + 10 },
                        head: [[ 'Departamento', 'Persona', 'Fecha', 'Evento', 'Estado' ]],
                        body,
