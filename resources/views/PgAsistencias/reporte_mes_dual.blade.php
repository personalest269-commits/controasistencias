...
@section('head')
    ...
    <style>
        ...
        .mark-a, .mark-ai, .mark-j, .mark-n, .mark-mix{ font-weight:800; display:inline-block; margin-right:2px; }
        .mark-a{ color:#16a34a; }
        .mark-ai{ color:#e0a800; }
        .mark-j{ color:#2563eb; }
        .mark-n{ color:#dc2626; }
        ...
        .event-item.event-ai .event-status{ color:#e0a800; }
...
<div class="container-fluid">
    <div class="row mb-3 no-print">
        <div class="col-12 d-flex flex-wrap justify-content-between align-items-center" style="gap:10px;">
            <div>
                <h4 class="mb-0">Asistencia por Mes (Modo Dual)</h4>
            ...
            <div class="d-flex" style="gap:8px;">
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('PgAsistenciasReportesDual') }}">Volver a Reportes Dual</a>
    ...
    <div class="card mb-3 no-print" style="border-radius:12px; border:0; box-shadow:0 10px 25px rgba(0,0,0,.06);">
        <div class="card-body">
            <form method="GET" action="{{ route('PgAsistenciasReporteMesDual') }}">
                <div class="row">
                    <div class="col-md-2">
                        <label>Año</label>
                        @php
                                ...
                                @foreach($m['weeks'] as $week)
                                    @foreach($week as $dateStr)
                                        @php
                                            ...
                                            $statusCounts = ['A' => 0, 'AI' => 0, 'F' => 0, 'J' => 0];
                                            foreach($eventCodes as $eventCodeTmp){
                                                $eventCodeNorm = strtoupper(ltrim(strip_tags((string)$eventCodeTmp)));
                                                if (preg_match('/\((AI|A|F|J)\)/i', $eventCodeNorm, $ms)) {
                                                    $statusCounts[strtoupper($ms[1])]++;
                                                } elseif (preg_match('/^(AI|A|F|J)(?:\\b|[^A-Z])/', $eventCodeNorm, $ms)) {
                                                    $statusCounts[strtoupper($ms[1])]++;
                                                } elseif (in_array($mark, ['A','AI','F','J'], true)) {
                                                    $statusCounts[$mark]++;
                                                }
                                            }
                                        @endphp
                                        <td class="w-day">
                                            @if($mark)
                                                @if(array_sum($statusCounts) > 0)
                                                    ...
                                                    @if($statusCounts['AI'] > 0)<span class="mark-ai">AI({{ $statusCounts['AI'] }})</span>@endif
                                                    ...
                                                    <div class="event-code" title="{{ implode(' | ', $eventCodes) }}">
                                                        @foreach($eventCodes as $eventCode)
                                                            @php
                                                                ...
                                                                $status = '';
                                                                if (preg_match('/\((AI|A|F|J)\)/', $eventCodeNorm, $mch)) {
                                                                    $status = strtoupper($mch[1]);
                                                                } elseif (preg_match('/^(AI|A|F|J)(?:\\b|[^A-Z])/', $eventCodeNorm, $mch)) {
                                                                    $status = strtoupper($mch[1]);
                                                                } elseif (in_array($mark, ['A','AI','F','J'], true)) {
                                                                    $status = $mark;
                                                                }
                                                                $eventClass = $status === 'A' ? 'event-a' : ($status === 'AI' ? 'event-ai' : ($status === 'J' ? 'event-j' : ($status === 'F' ? 'event-f' : 'event-x')));
                                                                $eventText = preg_replace('/\((AI|A|F|J)\)/i', '', $eventCode);
                                                                $eventText = preg_replace('/^\s*(AI|A|F|J)[\s\-\:\.]+/i', '', $eventText);
                                                                $eventText = preg_replace('/^\s*(AI|A|F|J)\s*/i', '', $eventText);
