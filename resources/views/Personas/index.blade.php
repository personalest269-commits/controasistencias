            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:10px;">
                        <form method="GET" action="{{ route('PersonasIndex') }}" class="form-inline" style="gap:8px;">
                            ...
                            <select name="empresa_id" id="filtro_empresa" class="form-control" style="min-width:260px;">
                                ...
                                @foreach(($empresas ?? []) as $e)
                                    <option value="{{ $e->id }}" {{ ($empresaId ?? null) == $e->id ? 'selected' : '' }}>
                                        {{ $e->nombre }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @endif
                            @if(!empty($soloInactivos))
                                <input type="hidden" name="inactivos" value="1" />
                            @endif
                            <button class="btn btn-secondary" type="submit">Buscar</button>
                            @if($q || ($departamentoId ?? null) || ($empresaId ?? null))
                                <a class="btn btn-light" href="{{ route('PersonasIndex', array_filter([
                        ...
                        <div>
                            <a class="btn btn-secondary" href="{{ route('PersonasIndex', $soloEliminados ? array_filter(['inactivos' => !empty($soloInactivos) ? 1 : null]) : array_filter(['eliminados' => 1, 'inactivos' => !empty($soloInactivos) ? 1 : null])) }}">
                                {{ $soloEliminados ? 'Ver activos' : 'Ver eliminados' }}
                            </a>

                            <a class="btn btn-secondary" href="{{ route('PersonasIndex', !empty($soloInactivos) ? array_filter(['eliminados' => $soloEliminados ? 1 : null]) : array_filter(['inactivos' => 1, 'eliminados' => $soloEliminados ? 1 : null])) }}">
                                {{ !empty($soloInactivos) ? 'Ver activos' : 'Ver inactivos' }}
                            </a>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="empresa_id" id="empresa_id_create" required class="form-control {{ $errors->has('empresa_id') ? 'is-invalid' : '' }}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="departamento_id" id="departamento_id_create" class="form-control {{ $errors->has('departamento_id') ? 'is-invalid' : '' }}">
                            <option value="">-- Seleccione --</option>
                            @foreach(($departamentos ?? []) as $d)
                                <option value="{{ $d->id }}" data-empresa-id="{{ $d->empresa_id }}" {{ old('departamento_id') == $d->id ? 'selected' : '' }}>
                                    {{ $d->descripcion }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="form-group">
                @if($theme === 'gentelella')
                    <label style="font-weight:normal;">
                        <input type="checkbox" id="crearUsuarioCheck" name="crear_usuario" value="1" onchange="toggleUsuarioNuevo(this);" {{ old('crear_usuario') ? 'checked' : '' }}>
                        Crear usuario
                    </label>
                @else
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="crearUsuarioCheck" name="crear_usuario" value="1" onchange="toggleUsuarioNuevo(this);" {{ old('crear_usuario') ? 'checked' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="usuario_new" name="usuario" class="form-control {{ $errors->has('usuario') ? 'is-invalid' : '' }}" placeholder="Se llena con la identificación" value="{{ old('usuario') }}" readonly>
...
(function () {
    ...
    function syncDepartamentosNuevoByEmpresa(forceClearSelection) {
        var empresa = document.getElementById('empresa_id_create');
        var dept = document.getElementById('departamento_id_create');
        ...
        if (forceClearSelection) {
            ...
            if (window.jQuery && $.fn.select2) {
                $('#departamento_id_create').val(null).trigger('change');
            }
        }
        ...
        if (selectedValue && !hasSelectedVisible) {
            dept.value = '';
            if (window.jQuery && $.fn.select2) {
                $('#departamento_id_create').val('').trigger('change.select2');
            }
        } else if (window.jQuery && $.fn.select2) {
            $('#departamento_id_create').trigger('change.select2');
        }
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var empresa = document.getElementById('empresa_id_create');
    syncDepartamentosNuevoByEmpresa(false);
    if (empresa) {
        empresa.addEventListener('change', function () {
            syncDepartamentosNuevoByEmpresa(true);
        });
    }
});
